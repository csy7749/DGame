...
- 新增代码必须根据职责添加清晰、简洁的中文注释。
- 类、接口、重要字段和公开方法应使用中文注释说明用途与职责。
- 复杂分支、状态转换、生命周期、异步流程、资源释放和非显然算法必须说明设计意图。
- 注释应解释“为什么这样做”以及关键约束，不要逐行翻译代码。
- 简单赋值、明显的条件判断和自解释代码不添加冗余注释。
- 修改既有代码时，只补充与本次修改直接相关的注释，不批量改写无关注释。
- 自动生成文件不手动添加注释；需要注释时修改生成模板。
- 注释必须与代码行为保持一致，行为变化时同步更新相关注释。
