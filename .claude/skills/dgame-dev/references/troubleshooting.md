...
Editor 走 Mono 或编辑器程序集，真机走 IL2CPP + HybridCLR。检查 `dynamic`、Emit、表达式树 `Compile()`、复杂反射泛型、Marshal/PInvoke、热更侧 Unity Attribute 自动执行等限制。
...
3. `ConfigSystem.Instance.Tables` 是否已初始化。
4. 业务是否改动了生成代码。
5. Excel、`__tables__.xlsx`、`__beans__.xlsx`、`__enums__.xlsx`、导表脚本问题优先使用 `luban-dev`。
...
`UniTaskVoid` 异常不按普通 `await` 链传播。优先让业务方法返回 `UniTask` 并在调用点 await；必须 fire-and-forget 时在方法内 try-catch。
