...
2. 拉取代码 → 转表（`GameConfig` 导表到 `BundleAssets/Configs/Binary/`，见 luban-dev）→ 调用对应 `build_*` 脚本。
3. `-quit` 后用退出码判断成功失败，`BUILD_LOGFILE` 归档；`.sh` 末尾的 `read` 交互仅本地用，CI 节点用 `_auto` 脚本或去掉暂停。
4. 首次或改 AOT 后必须先出一次整包再打 AB——裁剪后的 AOT DLL 在 `BuildPlayer` 时才生成（见 [hotfix-workflow.md](hotfix-workflow.md) AOT 泛型补充步骤）。
