...
当需求涉及客户端如何发起请求、接收响应、处理服务器推送、组织 `NetworkMgr` 或梳理 UI 到网络层的交互职责时，先读取 `references/client-server-communication-claude.md`。
...
当需求涉及帧同步相关的定点数学、定点物理、确定性随机数、`FixedPointPhysics`、`GameBattle` 中的位置/旋转/碰撞建模时，先读取 `references/frame-sync-fixedpoint-foundation-claude.md`。
...
2. `references/client-architecture-claude.md` 或 `references/server-architecture.md`
3. `references/client-server-communication-claude.md` 或 `references/proto-message-define.md`
4. `references/luban-game-config-claude.md`
...
先检查 `references/proto-message-define.md` 和 `references/client-server-communication-claude.md`。处理协议和网络链路时优先遵循：
