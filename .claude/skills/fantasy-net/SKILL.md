...
| 模块 | 实际路径 |
|------|----------|
| Unity 客户端 Fantasy 运行时 | `GameUnity/Assets/Scripts/HotFix/Fantasy.Unity/Runtime`（核心）、`Editor`（编辑器） |
| Unity 业务层（连接 / Session / 消息收发） | `GameUnity/Assets/Scripts/HotFix/GameLogic`（如 `DataCenter/GameClient.cs`） |
| 战斗 ECS（Entity / Component / System） | `GameUnity/Assets/Scripts/HotFix/GameBattle`（Battle / Unit / Event / FrameSync / Singleton） |
| 服务端三段式 | `GameServer/Server/Main`、`GameServer/Server/Entity`、`GameServer/Server/Hotfix` |
| 服务端 Scene 代码 | `GameServer/Server/Entity/Scene`、`GameServer/Server/Hotfix/Scene` |
| 协议定义（Outer / Inner） | `GameServer/Tools/NetworkProtocol/Outer`、`GameServer/Tools/NetworkProtocol/Inner` |
| 协议导出工具 | `GameServer/Tools/ProtocolExportTool` |
