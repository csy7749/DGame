...
namespace Procedure
{
    ...
    public class InitResourceProcedure : ProcedureBase
    {
        ...
        private async UniTaskVoid InitResources()
        {
            ...
            if (Utility.PlayerPrefsUtil.HasKey("GAME_VERSION"))
            {
        ...
        private bool IsNeedUpdate()
        {
            ...
            {
                // 获取上次成功记录的版本
                string packageVersion = Utility.PlayerPrefsUtil.GetString("GAME_VERSION", string.Empty);
                if (string.IsNullOrEmpty(packageVersion))
                {
                    // 之前没下载过游戏资源 第一次进入游戏需要下载资源
                    LauncherMgr.ShowUI(UIDefine.LoadUpdateUI, "当前网络不可用，请检查本地网络设置后点击确认进行重试");
                ...
                {
                    // 有更新提示 但不强制更新
                    LauncherMgr.ShowUI(UIDefine.LoadUpdateUI, "检测到可选资源更新，推荐完成更新提升游戏体验");
