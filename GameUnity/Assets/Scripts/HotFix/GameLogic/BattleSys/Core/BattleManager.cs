...
namespace GameLogic
{
    ...
    public static class BattleManager
    {
        ...
        public static void SubscribeBattleScoped<T>(object owner, SubscriptionScopeComponent scope,
            Action<T> handler) where T : struct, IBattleEvent
            => CurBattleContext.SubscribeBattleScoped(owner, scope, handler);
        ...
        /// <param name="owner">监听所属者。</param>
        /// <param name="handler">事件回调。</param>
        public static void SubscribeBattle<T>(object owner, Action<T> handler) where T : struct, IBattleEvent
            => CurBattleContext.SubscribeBattle(owner, handler);
        ...
        /// <typeparam name="T">战斗事件类型。</typeparam>
        /// <param name="handler">事件回调。</param>
        public static void UnsubscribeBattle<T>(Action<T> handler) where T : struct, IBattleEvent
            => CurBattleContext.UnsubscribeBattle(handler);
        ...
        public static void PublishBattle<T>(T eventData) where T : struct, IBattleEvent
            => CurBattleContext.PublishBattle(eventData);

        /// <summary>
        /// 移除指定所属者的全部战斗级事件监听。
        /// </summary>
        /// <param name="owner">监听所属者。</param>
        public static void RemoveAllBattleSubscriptions(object owner) => CurBattleContext.RemoveAllBattleSubscriptions(owner);
