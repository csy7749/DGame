...
namespace DGame
{
    ...
    {
        private const string stylesResource = "Assets/Editor/ShaderVariantCollection/ShaderCollectionUss/ShaderCollection.uss";
        ...
        private const string defaultShaderVariantCollectionPath =
            "Assets/Resources/Shaders/" + shaderVariantCollectionName;
        ...
        [SerializeField] private ShaderVariantCollectionMapper m_CollectionMapper;
        ...
        private ShaderVariantCollectionMapper collectionMapper
        {
            get
            {
                ...
                {
                    m_CollectionMapper = ScriptableObject.CreateInstance<ShaderVariantCollectionMapper>();
                }
        ...
        [MenuItem("DGame Tools/Shader变体收集器")]
        ...
        private void OnEnable()
        {
            ...
            root.styleSheets.Add(AssetDatabase.LoadAssetAtPath<StyleSheet>(stylesResource));
            ...
            m_SVCField.value =
                AssetDatabase.LoadAssetAtPath<ShaderVariantCollection>(defaultShaderVariantCollectionPath);
            m_SVCField.style.marginLeft = 20;
            m_SVCField.style.marginRight = 20;
            // register a callback on the field when the value changes
            m_SVCField.RegisterValueChangedCallback(evt => { RefreshShaderList(); });
            ...
            {
                string shaderVariantCollectionPath = m_SVCField.value
                    ? AssetDatabase.GetAssetPath(m_SVCField.value)
                    : defaultShaderVariantCollectionPath;
                ...
                ShaderCollection.shaderCollectionConfigAssets =
                    m_ShaderVariantCollectionField.value as ShaderCollectionAssets;
        ...
        private void DrawLeftContainer()
        {
            ...
            m_ShaderVariantCollectionField.value = AssetDatabase.FindAssets("t:ShaderCollectionAssets")
                .Select(guid =>
                    AssetDatabase.LoadAssetAtPath<ShaderCollectionAssets>(AssetDatabase.GUIDToAssetPath(guid)))
                .FirstOrDefault();
        ...
        private void UpdateShaderListUI()
        {
            ...
            var sortButton1 = new Button(() => { SortShaderList(0); })
            {
                text = "按名称排序"
            };
            sortButton1.AddToClassList(sortButtonClass);
            var sortButton2 = new Button(() => { SortShaderList(1); })
            {
                text = "按变体数量排序"
            };
        ...
        private void SortShaderList(int index)
        {
            ...
            else if (index == 1)
            {
                ...
                m_ShaderListData.Sort((a, b) =>
                {
                    ...
                    // 根据revertSort来决定升序还是降序
                    return revertSort
                        ? shaderAVariants.Count.CompareTo(shaderBVariants.Count)
                        : shaderBVariants.Count.CompareTo(shaderAVariants.Count);
                });
            }
        ...
        private void UpdateShaderVariantListUI(Shader currentShader)
        {
            ...
            foreach (var variantDict in m_VariantDict)
            {
                ...
                Action<VisualElement, int> bindItem = (e, i) =>
                {
                    ...
                    var shaderKeywordsStr = shaderVariant.keywords.Length == 0
                        ? "<No Keywords>"
                        : string.Join("  ", shaderVariant.keywords);
                    var label = e.Q<Label>();
            ...
            {
                ShaderVariantAddWindow.ShowWindow(CurrentSelectedShader, PassType.ScriptableRenderPipeline,
                    collectionMapper);
            })
            {
                text = "+"
        ...
        {
            var selectedVisualElement =
                m_ShaderList.Q<VisualElement>(className: "unity-collection-view__item--selected");
